...
import { 
  ...
  Package, ShoppingCart, CheckCircle2, Archive, Copy, RefreshCw
} from 'lucide-react';
...
const App = () => {
  ...
  const handleArchiveEvent = async (eventId, status) => {
    await updateDoc(doc(db, 'artifacts', APP_ID, 'users', user.uid, 'events', eventId), { archived: status });
  ...
  const handleCopyEvent = async (event) => {
    ...
    if (eventParticipants[event.id]) {
      await setDoc(doc(db, 'artifacts', APP_ID, 'users', user.uid, 'participants', newEvent.id), { ids: eventParticipants[event.id] });
    }
    if (eventBudgets[event.id]) {
      await setDoc(doc(db, 'artifacts', APP_ID, 'users', user.uid, 'budgets', newEvent.id), eventBudgets[event.id]);
    }
  };
          ...
          <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {events.filter(e => !!e.archived === showArchived).map(event => {
                ...
                return (
                  <div key={event.id} className="bg-slate-900 p-6 rounded-[1.5rem] border border-slate-800 hover:border-indigo-500/50 flex flex-col justify-between group transition-all shadow-md relative overflow-hidden">
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <button 
                ...
                className="w-full sm:w-auto flex items-center justify-center gap-2 text-[10px] md:text-xs font-black uppercase text-indigo-400 bg-indigo-400/10 px-4 py-2 rounded-xl border border-indigo-500/10 transition-all hover:bg-indigo-400/20"
              >
                {isBudgetsCollapsed ? <><Settings2 className="w-4 h-4" /> Budgets</> : <><LayoutList className="w-4 h-4" /> Liste</>}
              </button>
            </div>
            
            <div className="bg-gradient-to-br from-indigo-600 to-indigo-800 p-8 md:p-10 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl relative overflow-hidden">
            ...
            <div className={`grid grid-cols-1 ${isBudgetsCollapsed ? 'lg:grid-cols-1' : 'lg:grid-cols-3'} gap-8 items-start`}>
              {!isBudgetsCollapsed && (
                <div className="lg:col-span-1 space-y-4 animate-in slide-in-from-left-4 duration-500">
                  <h3 className="text-xs font-black uppercase text-slate-500 px-2 tracking-widest">Budgets</h3>
                  {givers.map(giver => {
                    ...
                    return (
                      <div key={giver.id} className={`p-4 rounded-2xl border transition-all ${isPart ? 'bg-slate-900 border-indigo-500/30 shadow-sm' : 'bg-slate-950 border-slate-800 opacity-40'}`}>
              ...
              <div className={`${isBudgetsCollapsed ? 'lg:col-span-1' : 'lg:col-span-2'} space-y-6`}>
                <section className="bg-slate-900 p-6 rounded-[2rem] border border-slate-800 shadow-xl">
                  ...
                  <form onSubmit={async (e) => {
                    ...
                    <input name="name" placeholder="Geschenk / Idee" required className="sm:col-span-5 p-4 bg-slate-800 border border-slate-700 rounded-2xl text-base outline-none focus:ring-2 focus:ring-indigo-500 transition-all" />
                    <input name="price" type="number" step="0.01" placeholder="Preis €" required className="sm:col-span-2 p-4 bg-slate-800 border border-slate-700 rounded-2xl text-base outline-none focus:ring-2 focus:ring-indigo-500 transition-all font-mono" />
                    <select name="giverId" className="sm:col-span-3 p-4 bg-slate-800 border border-slate-700 rounded-2xl text-xs outline-none focus:ring-2 focus:ring-indigo-500 transition-all">
                      <option value="">Noch offen</option>
                      {activeGiversForEvent.map(g => <option key={g.id} value={g.id}>{g.name}</option>)}
                    </select>
                    <button type="submit" className="sm:col-span-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-2xl font-black shadow-lg transition-all py-3 flex items-center justify-center"><Plus className="w-8 h-8" /></button>
                  </form>
                </section>
                    ...
                    <div key={gift.id} className="bg-slate-900 p-5 rounded-[1.5rem] border border-slate-800 flex flex-col gap-4 shadow-sm group">
                      {editingGiftId === gift.id ? (
                        <div className="flex flex-col gap-4">
                          <input className="p-4 bg-slate-800 border border-indigo-500/50 rounded-2xl text-white text-base outline-none" value={editFormData.name} onChange={(e) => setEditFormData({...editFormData, name: e.target.value})} />
                          ...
                          <div className="flex justify-end gap-3 mt-2">
                            ...
                            }} className="text-emerald-400 bg-emerald-400/10 px-6 py-2 rounded-xl text-sm font-black flex items-center gap-2"><Check className="w-4 h-4" /> Save</button>
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <div className="text-right flex items-center gap-4 ml-4">
                            <p className="text-xl font-black text-indigo-400 font-mono">{Number(gift.price).toFixed(2)}€</p>
                            <div className="flex gap-1">
                              <button onClick={() => { setEditingGiftId(gift.id); setEditFormData({ name: gift.name, price: gift.price, giverId: gift.giverId || '', status: gift.status || 'Idee' }); }} className="p-2 text-slate-600 hover:text-indigo-400"><Edit2 className="w-5 h-5" /></button>
                              <button onClick={() => deleteDoc(doc(db, 'artifacts', APP_ID, 'users', user.uid, 'gifts', gift.id))} className="p-2 text-slate-600 hover:text-red-500"><Trash2 className="w-5 h-5" /></button>
                            </div>
                          </div>
                        </div>
                      )}
